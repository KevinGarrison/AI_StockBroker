...
async def delete_cache():
    async with httpx.AsyncClient() as client:
        response = await client.get("http://localhost:8002/delete-cached-docs", timeout=None)#("http://rag-chatbot:8002/delete-cached-docs", timeout=None)
        return response.json()

@app.get("/first-filter-dropdown")
async def first_filter():
    async with httpx.AsyncClient() as client:
        response = await client.get("http://localhost:8001/dropdown-values", timeout=None)#("http://api-fetcher:8001/dropdown-values", timeout=None)
        return response.json()

@app.get("/second-filter-market-cap/{screener}/{min_cap}")
async def second_filter(screener:str, min_cap:int):
    async with httpx.AsyncClient() as client:
        response = await client.get(f"http://localhost:8001/filter-market-cap/{screener}/{min_cap}", timeout=None)#(f"http://api-fetcher:8001/filter-market-cap/{screener}/{min_cap}", timeout=None)
        return response.json()
...
async def companies():
    async with httpx.AsyncClient() as client:
        response = await client.get("http://localhost:8001/companies", timeout=None)#("http://api-fetcher:8001/companies", timeout=None)
        return response.json()

@app.get("/stock-history/{ticker}")
async def stock_history(ticker: str):
    async with httpx.AsyncClient() as client:
        response = await client.get(f"http://localhost:8001/stock-history/{ticker}", timeout=None)#(f"http://api-fetcher:8001/stock-history/{ticker}", timeout=None)
        return response.json()

@app.get("/company-facts/{ticker}")
async def company_facts(ticker: str):
    async with httpx.AsyncClient() as client:
        response = await client.get(f"http://localhost:8001/yfinance-company-facts/{ticker}", timeout=None)#(f"http://api-fetcher:8001/yfinance-company-facts/{ticker}", timeout=None)
        return response.json()

@app.get("/company-news/{ticker}")
async def company_news(ticker: str):
    async with httpx.AsyncClient() as client:
        response = await client.get(f"http://localhost:8001/company-news/{ticker}", timeout=None)#(f"http://api-fetcher:8001/company-news/{ticker}", timeout=None)
        return response.json()
...
async def get_reference_files():
    async with httpx.AsyncClient() as client:
        response = await client.get("http://localhost:8002/reference-docs-from-analysis", timeout=None)#(f"http://rag-chatbot:8002/reference-doc-from-analysis/{accession}/{filename}", timeout=None)     
        return response.json()

@app.get("/forecast/{ticker}")
async def get_forecast(ticker:str):
    async with httpx.AsyncClient() as client:
        response = await client.get(f"http://localhost:8003/forecast/{ticker}", timeout=None)#(f"http://forecasting:8003/forecast/{ticker}", timeout=None)     
        return response.json()
